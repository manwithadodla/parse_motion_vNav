...
def readRotAndTrans(paths):
  ...
  return list(itertools.chain.from_iterable([head, [(np.array(y[1:5]), y[6:9]) for y in [str.split(x.ImageComments) for x in ds[1:]]]]))
...
def motionEntryToHomogeneousTransform(e) :
  ...
  t[0:3,3] = np.fromiter(e[1], dtype=np.float)
  t[0:3,0:3] = quaternionToRotationMatrix(angleAxisToQuaternion(np.fromiter(e[0], dtype=np.float)))
  t[3,:] = [0,0,0,1]
  return np.matrix(t)
